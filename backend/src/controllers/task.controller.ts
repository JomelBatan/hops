...
async function ensureColumnBoard(columnId: string, boardId: string) {
  const { rows } = await query(
...
export async function listTask(req: Request, res: Response) {
  try {
    ...
    const params: string[] = [req.board?.id!];

    if (req.query.priority) {
      params.push(req.query.priority as string);
      filters.push(`t.priority = $${params.length}`);
    }
    if (req.query.assignee) {
      params.push(req.query.assignee as string);
      filters.push(`t.asignee_id = $${params.length}`);
    }
    if (req.query.column) {
      params.push(req.query.column as string);
    ...
    res
      .status(500)
      .json({ error: "Error on Task Controller: [List Task Function]" });
  }
}
...
export async function createTask(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error on Task Controller: [Create Task Function]" });
  }
}
...
export async function updateTask(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error on Task Controller: [Update Task Function]" });
  }
}
...
export async function moveTask(req: Request, res: Response) {
  try {
    ...
    await ensureColumnBoard(column_id, req.board?.id!);
    ...
    res
      .status(500)
      .json({ error: "Error on Task Controller: [Move Task Function]" });
  }
}
...
export async function deleteTask(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error on Task Controller: [Delete Task Function]" });
  }
}
