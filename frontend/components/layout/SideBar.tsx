...
function NavItem({ to, icon: Icon, label, collapsed, badge }: NavItemProps) {
  ...
  return (
    <Link
      ...
      className={`group relative flex h-11 items-center rounded-2xl text-sm font-medium transition-colors duration-200 ${
    ...
    >
        ...
        {loading
          ...
          : boards.length === 0
            ...
            : boards.map((b) => {
                ...
                // Compare current Next.js pathname to this link's destination
                ...
                return (
                  <Link
                    ...
                    href={href}
                    title={b.title}
                    // Remove the callback function and pass the result directly to cn()
                    className={`
      ...
      <nav className="space-y-1 px-3">
        ...
        <button
          ...
          className={`"group flex h-11 w-full items-center rounded-2xl text-sm font-medium text-muted transition-colors duration-200 hover:bg-surface-2 hover:text-ink
           ${collapsed ? "mx-auto w-11 justify-center" : "gap-3 px-3"}`}
        >
