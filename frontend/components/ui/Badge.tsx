...
export const PriorityBadge = ({ priority, className = "" }: PriorityProps) => {
...
export const PriorityTag = ({ priority, className = "" }: PriorityProps) => {
  const meta = priorityMeta(priority);
  return (
    <span
...
export const Badge = ({ children, className = "" }: BadgeProps) => (
  <span
    className={`inline-flex items-center gap-1 rounded-full bg-surface-2 px-2.5 py-1 text-[11px] font-medium tracking-tight text-muted
      ${className}`}
  >
    {children}
  </span>
